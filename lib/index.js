...
var sendEmail = (params, callback) => __async(void 0, null, function* () {
  const _a = params, { type = "qq", name = "", smtp = "", from = "" } = _a, other = __objRest(_a, ["type", "name", "smtp", "from"]);
  let nodeMail = import_nodemailer.default.createTransport(__spreadProps(__spreadValues({}, types[type]), {
    auth: {
      user: from,
      pass: smtp
    }
  }));
  const mail = __spreadValues({
    from: `"${name}"<${from}>`
  }, other);
  yield nodeMail.sendMail(mail, (err, info) => {
    callback(!err, info);
  });
});
var nodeSendEmail = { sendEmail };
var lib_default = nodeSendEmail;
